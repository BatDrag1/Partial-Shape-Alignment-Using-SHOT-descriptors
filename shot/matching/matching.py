...
def get_n_best_correspodences(scan_descriptors,ref_descriptors,n,threshold,method):
    """
    Args:
        scan: descriptors from point cloud to be transformed
        ref: descriptors from point cloud as reference
        n: number of pairs (different) that will be found
    """
    non_empty_descriptors = np.any(scan_descriptors, axis=1).nonzero()[0]
    ...
    indices = distance_matrix.argmin(axis=1)
    distances = distance_matrix[np.arange(non_empty_descriptors.shape[0]), indices]
    data_set = sorted([(x,y,z) for x,y,z in zip(range(nr_of_point_in_scan),
                                                      indices,
                                                      distances)],
                      key = lambda x:x[2])
    continues = True
    ...
    k = 0
    while continues:
        if k == nr_of_point_in_scan - 1:
            continues = False
        elif len(trimmed_data_set) < 250:
            trimmed_data_set.append(data_set[k])
        elif (data_set[0][2] / data_set[k][2] > threshold):
            trimmed_data_set.append(data_set[k])
        else:
            continues = False
        k = k + 1
    correspondences = []
    print("Amount of pairs found in the matching stage: "+str(k))
    for j in range(len(trimmed_data_set)):
        liste = np.argpartition(distance_matrix[trimmed_data_set[j][0]],n)[:n]
        correspondences.append((trimmed_data_set[j][0],list(liste)))
